...
set.seed(111) # for random initialisation
...
stan_model <- stan_model("Models/Model_infected.stan")
...
df_run <- df_run |>
  arrange(ID, time) |>
  filter(Tag_tinf == 1) |> # filter individuals with >= 1 Pcr positive
  as.data.frame()
...
inits_to_print <- lapply(inits_list, function(x) x[1:5])
...
save(fit, file = "fit_Scenario1_only_infected_1.Rsave")
